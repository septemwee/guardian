...
const NavChat = ({ messageText, setMessageText, onSend, onToggleSender, currentSender }: NavChatProps) => {
  // กำหนดสีไอคอนตามผู้ส่งปัจจุบัน
  const iconFillColor = currentSender === 'user' ? '#007AFF' : '#FF0303';

  return (
    // เปลี่ยนจาก KeyboardAvoidingView เป็น View ธรรมดา
    // เนื่องจาก Chatpage ซึ่งเป็นคอมโพเนนต์แม่ได้จัดการ KeyboardAvoidingView ไว้แล้ว
...
const styles = StyleSheet.create({
  ...
  inputContainer: {
    flex: 1, // ใช้ flex: 1 เพื่อให้ View นี้ขยายเต็มพื้นที่ที่เหลือ
    position: 'relative',
    flexDirection: 'row',
    alignItems: 'center',
    borderWidth: 1,
    borderColor: '#8D8B8B',
    borderRadius: 30,
    paddingRight: width * 0.1, // กำหนด paddingRight เพื่อให้ไอคอนด้านในมีพื้นที่
  },
  textInput: {
    flex: 1, // ใช้ flex: 1 เพื่อให้ TextInput ขยายเต็มพื้นที่ที่เหลือภายใน inputContainer
    height: '100%',
    paddingVertical: 8,
    paddingLeft: 16,
  },
